...
# Hyperparameters
learning_rate = [1.0, 0.5, 0.1, 0.05, 0.01, 0.005, 0.001]
num_epochs = 1000
batch_size = 1

train_mse = Matrix{Float32}(length(learning_rate), 1)
val_mse = Matrix{Float32}(length(learning_rate), 1)
test_mse = Matrix{Float32}(length(learning_rate), 1)
train_acc = Matrix{Float32}(length(learning_rate), 1)
val_acc = Matrix{Float32}(length(learning_rate), 1)
test_acc = Matrix{Float32}(length(learning_rate), 1)
stopping_time = Matrix{Float32}(length(learning_rate), 1)

for l in 1:length(learning_rate)
    println(l, " / ", length(learning_rate))
    runs = 5
    ...
    for k in 1:runs
        ...
        for i in 1:num_epochs
            ...
            for j in 1:train_size
                ...
                optimize!(optim, loss, step_size=learning_rate[l])
            end
...
function plot_mse()
    pyplot()
    plot(train_mse[:, 1], color=:blue, label="training mse", xticks=(1:length(learning_rate), map(string, learning_rate)))
    plot!(val_mse[:, 1], color=:orange, label="validation mse")
    plot!(test_mse[:, 1], color=:green, label="test mse")
    title!("MSE by Learning Rate for Vowels Dataset")
    xaxis!("Learning Rate")
    yaxis!("MSE")
    # savefig("img/VowelLRMSE.png")
end

function plot_acc()
    pyplot()
    plot(train_acc[:, 1], color=:blue, label="training accuracy", xticks=(1:length(learning_rate), map(string, learning_rate)))
    plot!(val_acc[:, 1], color=:orange, label="validation accuracy")
    plot!(test_acc[:, 1], color=:green, label="test accuracy")
    title!("Accuracy by Learning Rate for Vowels Dataset")
    xaxis!("Learning Rate")
    yaxis!("Accuracy")
    # savefig("img/VowelLRAccuracy.png")
end

function plot_stopping_time()
    pyplot()
    plot(stopping_time, color=:blue, label="stopping time", xticks=(1:length(learning_rate), map(string, learning_rate)))
    title!("Stopping Time by Learning Rate for Vowels Dataset")
    xaxis!("Learning Rate")
    yaxis!("Stopping Time (Epochs)")
    # savefig("img/VowelLRStoppingTime.png")
end
