...
num_epochs = 1000
batch_size = 1
momentum_term = [0.1, 0.25, 0.5, 0.75, 0.9]
num_tests = length(momentum_term)
...
for l in 1:num_tests
    ...
    for k in 1:runs
        # Layers
        fc1, Wb1 = fc_layer("Layer1", num_features, 100)
        fc2, Wb2 = fc_layer("Layer2", 100, num_classes, activation_fn=softmax)
        optim = SGDWithMomentum(momentum_term[l])
...
function plot_mse()
    pyplot()
    plot(train_mse[:, 1], color=:blue, label="training mse", xticks=(1:length(momentum_term), map(string, momentum_term)))
    plot!(val_mse[:, 1], color=:orange, label="validation mse")
    plot!(test_mse[:, 1], color=:green, label="test mse")
    title!("MSE by Momentum Term for Vowels Dataset")
    xaxis!("Gamma")
    yaxis!("MSE")
...
function plot_acc()
    pyplot()
    plot(train_acc[:, 1], color=:blue, label="training accuracy", xticks=(1:length(momentum_term), map(string, momentum_term)))
    plot!(val_acc[:, 1], color=:orange, label="validation accuracy")
    plot!(test_acc[:, 1], color=:green, label="test accuracy")
    title!("Accuracy by Momentum Term for Vowels Dataset")
    xaxis!("Gamma")
    yaxis!("Accuracy")
...
function plot_stopping_time()
    pyplot()
    plot(stopping_time, color=:blue, label="stopping time", xticks=(1:length(momentum_term), map(string, momentum_term)))
    title!("Stopping Time by Momentum Term for Vowels Dataset")
    xaxis!("Gamma")
